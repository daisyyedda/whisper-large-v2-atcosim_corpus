...
def filterNonEnglishCharacters(text):
    """
    Remove any non-English characters, allowing only ASCII character set.
    This includes English letters, numerals, and basic punctuation.
    """
...
def filterAndNormalize(text):   
    text = filterNonEnglishCharacters(text)  # Add this line to filter out non-English characters first
    ...
    text = normalizer(text)  # Running twice because the normalizer will replace 'zero five' by '05' but also replaces '05' by '5' (removing leading zeros).
